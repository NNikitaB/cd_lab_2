   ...
   if(p1->size>2 && p2->size>2)
{
    ...
    do
    {
        basetype ttm,ttt;
          ...
          Listread(p2,&pt3);
          ttm=comp_vect(pt1,pt2);
          ttm.z=pt2.z-pt1.z;
          ttt=comp_vect(pt1,pt3);
          ttt.z=pt3.z-pt1.z;
          double ki=ttt.y*ttm.z-ttm.y*ttt.z,kj=-(ttt.x*ttm.z-ttm.z*ttt.z),kk=ttt.x*ttm.y-ttm.x*ttt.y;

          if( numEuquel((ki*ki+kj*kj+kk*kk),0.0)==1 )// numEuquel(sqrt(ki*ki+kj*kj+kk*kk),0.0)==0
           {if(! (point_in_plane_vect(pt1,pt2,pt3) &&( (min(pt1.z,pt2.z)<=pt3.z) && (pt3.z<=max(pt1.z,pt2.z)) ) )  )break;}
           else break;

    }while( 0<rt  );
    if(rt<=0){printf("points in one line \n");return tmp;}

   arr=comp_PL(pt1,pt2,pt3,&flag);
   if( !(points_in_PL(arr,p1) && points_in_PL(arr,p2) )){printf("\n\n ERROR WRITEs \n\n");return tmp;}
   if(flag==OX || flag==OY)rewrite_poligon(p1,flag),rewrite_poligon(p2,flag),printf("\n\n ERROR+++  \n\n");
...
double * comp_PL(basetype a, basetype b,basetype c,int * flag)
{
    *flag=OXYZ;
    ...
    else  if(numEuquel(arr[1],0.0) && numEuquel(arr[2],0.0))*flag=OX;//плоскость || ZOY
           else if(numEuquel(arr[2],0.0) && numEuquel(arr[0],0.0))*flag=OY;//плоскость || XOY
